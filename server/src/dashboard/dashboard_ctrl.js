...
controller.getUltimasExecucoes = async (total = 10) => {
  return db.conn.any(
    ...
    INNER JOIN edicao.versao_rotina AS vr ON vr.id = e.versao_rotina_id
    INNER JOIN edicao.rotina AS r ON r.id = e.rotina_id
...
controller.getRotinas = async () => {
  const result = await db.conn.one(
    'SELECT count(*) FROM edicao.rotina WHERE ativa IS TRUE'
  )
...
const getExecRotinaByStatus = async (status, total, max) => {
  const rotinas = await db.conn.any(
    ...
    FROM edicao.execucao AS e
    LEFT JOIN edicao.rotina AS r ON r.id = e.rotina_id
